...
template <class T> bool compareArrays(Graph graph, T *ref, T *stu) {
...
template <class T> bool compareApprox(Graph graph, T *ref, T *stu) {
    for (int i = 0; i < graph->num_nodes; i++) {
...
template <class T> bool compareArraysAndDisplay(Graph graph, T *ref, T *stu) {
    printf("\n----------------------------------\n");
...
template <class T> bool compareArraysAndRadiiEst(Graph graph, T *ref, T *stu) {
    bool isCorrect = true;
    for (int i = 0; i < graph->num_nodes; i++) {
        if (ref[i] != stu[i]) {
            std::cerr << "*** Results disagree at " << i << " expected "
